    ...
    try:
        with zipfile.ZipFile(path) as archive:
            ...
            if len(infos) > policy.max_container_entries:
                raise ExtendedFormatValidationError(
                    "CONTAINER_RESOURCE_LIMIT", "too many container entries"
                )
            total = 0
            names: set[str] = set()
            for info in infos:
                ...
                if name.startswith("/") or any(part == ".." for part in name.split("/")):
                    raise ExtendedFormatValidationError(
                        "CONTAINER_PATH_TRAVERSAL", "unsafe container entry path"
                    )
                if name in names:
                    raise ExtendedFormatValidationError(
                        "CONTAINER_DUPLICATE_ENTRY", "duplicate container entry name"
                    )
                names.add(name)
                if info.flag_bits & 0x1:
                    raise ExtendedFormatValidationError(
                        "CONTAINER_ENCRYPTED", "encrypted container entries are unsupported"
                    )
                if info.file_size > policy.max_single_entry_uncompressed_bytes:
                    raise ExtendedFormatValidationError(
                        "CONTAINER_RESOURCE_LIMIT", "container entry too large"
                    )
                total += info.file_size
                if total > policy.max_total_uncompressed_bytes:
                    raise ExtendedFormatValidationError(
                        "CONTAINER_RESOURCE_LIMIT", "container uncompressed total too large"
                    )
                if info.file_size / max(info.compress_size, 1) > policy.max_compression_ratio:
                    raise ExtendedFormatValidationError(
                        "CONTAINER_RESOURCE_LIMIT", "container compression ratio exceeds limit"
                    )
                lower = name.lower()
                if (
                    lower.endswith((".zip", ".docx", ".xlsx", ".pptx", ".odt", ".epub"))
                    and policy.max_nested_container_depth == 0
                ):
                    raise ExtendedFormatValidationError(
                        "CONTAINER_NESTING_LIMIT", "nested container processing is disabled"
                    )
            ...
            for fmt, mime, required in ZIP_FORMATS:
                if all(item in names for item in required):
                    if fmt == "EPUB":
                        value = (
                            archive.read("mimetype")[:64].decode("ascii", errors="ignore").strip()
                        )
                        if value != "application/epub+zip":
                            continue
                    if fmt == "ODT":
                        value = (
                            archive.read("mimetype")[:128].decode("ascii", errors="ignore").strip()
                        )
                        if value != "application/vnd.oasis.opendocument.text":
                            continue
                    return fmt, mime
            raise ExtendedFormatValidationError(
                "UNSUPPORTED_ZIP_CONTAINER", "ZIP package is not an admitted document format"
            )
    except ExtendedFormatValidationError:
        raise
    except (zipfile.BadZipFile, OSError) as exc:
        raise ExtendedFormatValidationError(
            "MALFORMED_CONTAINER", "container validation failed"
        ) from exc
            ...
            for byte in chunk:
                ...
                if char == "\\":
                    ...
                    if depth > max_group_depth:
                        raise ExtendedFormatValidationError(
                            "RTF_RESOURCE_LIMIT", "RTF group nesting exceeds limit"
                        )
                elif char == "}":
                    depth -= 1
                    if depth < 0:
                        raise ExtendedFormatValidationError(
                            "MALFORMED_RTF", "unbalanced RTF groups"
                        )
    if depth != 0:
...
def detect_html(sample: str, extension: str) -> bool:
    ...
    strong = lowered.startswith("<!doctype html") or bool(
        re.search(r"<html(?:\s|>)", lowered[:4096])
    )
