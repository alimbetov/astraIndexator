...
class SafeAcquisitionService:
        ...
        *,
        ...
        try:
            ...
            try:
                ...
                self.metrics.storage_request(
                    operation="head",
                    result="success",
                    duration_seconds=time.monotonic() - head_started,
                )
                ...
                raise AcquisitionError(
                    "STORAGE_CONNECT_TIMEOUT", "source storage connect timeout"
                ) from exc
                ...
                raise AcquisitionError(
                    "STORAGE_READ_TIMEOUT", "source storage read timeout"
                ) from exc

            if not head.exists:
                raise AcquisitionError(
                    "SOURCE_NOT_FOUND", f"source object does not exist: {ref.as_uri()}"
                )
            ...
            try:
                    ...
                    try:
                        for chunk in self.storage.iter_bytes(ref):
                            ...
                            if count > self.policy.max_source_bytes:
                                raise AcquisitionError(
                                    "SOURCE_TOO_LARGE", "streamed source exceeds configured maximum"
                                )
                            target.write(chunk)
                            digest.update(chunk)
                            if self.workspace_manager is not None:
                ...
                if head.size_bytes is not None and count != head.size_bytes:
                    raise AcquisitionError(
                        "SOURCE_SIZE_MISMATCH", "HEAD size differs from acquired byte count"
                    )

                sha = digest.hexdigest()
                normalized_expected = (
                    expected_sha256.removeprefix("sha256:") if expected_sha256 else None
                )
                if normalized_expected and normalized_expected.lower() != sha.lower():
                    raise AcquisitionError(
                        "SOURCE_CONTENT_MISMATCH",
                        "acquired SHA-256 differs from durable source hash",
                    )
            ...
            self.metrics.acquisition_failed(
                error_code=exc.code, duration_seconds=time.monotonic() - started
            )
            raise
    ...
    def _check_deadline(self, started: float) -> None:
        if time.monotonic() - started > self.policy.total_deadline_seconds:
            raise AcquisitionError(
                "ACQUISITION_DEADLINE_EXCEEDED", "total acquisition deadline exceeded"
            )
    ...
    def _validate(self, path: Path, original_name: str | None) -> tuple[str, str, list[str]]:
        ...
        if prefix.startswith(b"MZ") or prefix.startswith(b"\x7fELF"):
            raise AcquisitionError(
                "UNSUPPORTED_EXECUTABLE", "executable content is not a supported document"
            )
        if prefix.startswith(b"%PDF-"):
            if b"%%EOF" not in suffix:
                raise AcquisitionError(
                    "MALFORMED_PDF", "PDF EOF marker not found in bounded trailer probe"
                )
    ...
    def _validate_utf8_text(path: Path) -> None:
        ...
        try:
            with path.open("rb") as stream:
                while chunk := stream.read(1024 * 1024):
                    if probed < 1024 * 1024:
                        ...
                        if b"\x00" in probe:
                            raise AcquisitionError(
                                "UNSUPPORTED_FORMAT", "binary format is not supported"
                            )
                        probed += len(probe)
            ...
            raise AcquisitionError(
                "TEXT_ENCODING_UNSUPPORTED", "text source is not valid UTF-8"
            ) from exc
        ...
        try:
            with Image.open(path) as image:
                if image.format != expected:
                    raise AcquisitionError(
                        "IMAGE_FORMAT_MISMATCH", "image decoder disagrees with signature"
                    )
                ...
                if (
                    ...
                    raise AcquisitionError(
                        "IMAGE_RESOURCE_LIMIT", "image dimensions exceed configured limits"
                    )
                if (
                    expected == "TIFF"
                    and getattr(image, "n_frames", 1) > self.policy.max_tiff_pages
                ):
                    raise AcquisitionError(
                        "IMAGE_RESOURCE_LIMIT", "TIFF page count exceeds configured limit"
                    )
                image.verify()
