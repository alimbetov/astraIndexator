...
def _fingerprint(
    document: ParsedDocument, profile: OcrProfile, engine: OcrEngine, mode: OcrMode
) -> str:
...
def _project_observations(
    observations: tuple[OcrObservation, ...], candidate
...
def _normalized_geometry(
    geometry: SourceGeometry | None, resolved: ResolvedOcrInput
) -> SourceGeometry | None:
    if geometry is None:
        return None
    ...
    if source_region is not None and None not in (
        source_region.x0,
        source_region.y0,
        source_region.x1,
        source_region.y1,
        ...
        if source_region.coordinate_space == "normalized-0-1":
            rx0, ry0, rx1, ry1 = map(
                float, (source_region.x0, source_region.y0, source_region.x1, source_region.y1)
            )
...
def _normalize_observations(
    observations: tuple[OcrObservation, ...], resolved: ResolvedOcrInput
) -> tuple[OcrObservation, ...]:
...
def _validate_pipeline_inputs(
    source: AcquiredSource, document: ParsedDocument, profile: OcrProfile, engine: OcrEngine
) -> None:
...
class OcrPipelineService:
    ...
    def process(
        ...
        *,
        ...
        warnings: list[str] = []
        candidate_by_id = {
            candidate.candidate_id: candidate for candidate in document.ocr_candidates
        }
        image_cache: dict[str, tuple[OcrObservation, ...]] = {}

        for candidate in document.ocr_candidates:
            decision = self.policy.decide(
                document=document, candidate=candidate, mode=mode, profile=profile
            )
            self.metrics.candidate(decision.decision.value, candidate.scope)
            if decision.decision != OcrDecision.REQUIRED:
                results.append(
                    OcrCandidateResult(
                        candidate.candidate_id, decision.decision, decision.reason_codes
                    )
                )
                continue
            if time.monotonic() - started_job >= profile.timeout_per_job_seconds:
                ...
                results.append(
                    OcrCandidateResult(
                        ...
                        OcrDecision.REJECTED_RESOURCE_LIMIT,
                        ("OCR_JOB_TIMEOUT",),
                    )
                )
            ...
            try:
                resolved = self.resolver.resolve(
                    source=source, document=document, candidate=candidate, profile=profile
                )
            except Exception:
            ...
            try:
                ...
                if (
                    resolved.width < profile.min_image_width
                    or resolved.height < profile.min_image_height
                    or resolved.pixels < profile.min_image_pixels
                ):
                    results.append(
                        OcrCandidateResult(
                            candidate.candidate_id,
                            OcrDecision.NOT_REQUIRED,
                            ("DECORATIVE_IMAGE_TOO_SMALL",),
                        )
                    )
                    continue
                if (
                    resolved.pixels > profile.max_pixels_per_page
                    or pixels + resolved.pixels > profile.max_total_pixels_per_job
                ):
                ...
                if candidate.scope == "PAGE":
                    pages += 1
                retained = tuple(
                    obs for obs in observations if obs.confidence >= profile.hard_confidence_floor
                )
                if len(retained) < len(observations):
                    warnings.append("OCR_LOW_CONFIDENCE_DROPPED")
                results.append(
                    OcrCandidateResult(
                        candidate.candidate_id, decision.decision, tuple(reason_codes), retained
                    )
                )
            finally:
                if resolved.cleanup:
        ...
        for result in results:
            ...
            for observation in result.observations:
                ...
                duplicate_native = any(
                    _similarity(observation.text, text) >= 0.86 for text in native
                )
                ...
                element_id = hashlib.sha256(
                    ...
                    f"{self.engine.model_identity.bundle_sha256}\x1f{observation.text}".encode(
                        "utf-8"
                    )
                ).hexdigest()
                accepted.append(
        ...
        unresolved = any(
            r.decision in {OcrDecision.REQUIRED_BUT_DISABLED, OcrDecision.REJECTED_RESOURCE_LIMIT}
            or r.warnings
            for r in results
        )
        if unresolved:
        ...
        quality = ParseQuality(
            status,
            document.quality.native_text_chars,
            document.quality.ocr_candidate_count,
            tuple(dict.fromkeys((*document.quality.warnings, *warnings))),
        ...
        return OcrPipelineResult(
            ...
            tuple(results),
            tuple(accepted),
            _fingerprint(document, profile, self.engine, mode),
            pages,
            pixels,
