...
def upgrade() -> None:
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["job_id"],
            [f"{SCHEMA}.indexation_job.id"],
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint(
            "document_id",
            "document_version",
            name="pk_document_version_lifecycle",
        ),
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "requested_access_zone_code IS NULL OR "
            "requested_access_zone_code ~ '^[0-9]{4}$'",
            name="ck_lifecycle_operation_requested_access_zone_code_format",
        ),
        sa.CheckConstraint(
            "reason IS NULL OR length(trim(reason)) > 0",
            name="ck_lifecycle_operation_reason_non_blank",
        ),
        sa.ForeignKeyConstraint(
            ["job_id"],
            [f"{SCHEMA}.indexation_job.id"],
...
def downgrade() -> None:
    op.drop_index(
        "ix_lifecycle_operation_retry",
        table_name="lifecycle_operation",
        schema=SCHEMA,
    )
    op.drop_index(
        "ix_lifecycle_operation_document",
