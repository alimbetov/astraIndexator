...
MODEL = OcrModelIdentity("paddle-ru-kk-en", "paddleocr", "3.x", "r1", "bundle-sha", ("ru", "kk", "en"))
    ...
    def recognize(self, request):
        ...
        return tuple(
            OcrObservation(
                ...
                geometry=SourceGeometry(page_number=request.page_number, x0=1, y0=1, x1=10, y1=10,
...
class FakeResolver:
    ...
    def resolve(self, *, source, document, candidate, profile):
        return ResolvedOcrInput(candidate.candidate_id, self.image_path, self.width, self.height,
                                candidate.page_number, candidate.element_id, candidate.geometry, False)
...
def parsed(src: AcquiredSource, *, native_text=None, scope="PAGE", page_mode="SCANNED_IMAGE") -> ParsedDocument:
    image = DocumentElement("img", ElementType.IMAGE, 0, geometry=SourceGeometry(page_number=1), role="SCANNED_PAGE")
    elements = [image]
    if native_text:
        elements.insert(0, DocumentElement("native", ElementType.PARAGRAPH, 0, text=native_text,
                                           geometry=SourceGeometry(page_number=1)))
        elements[1] = DocumentElement("img", ElementType.IMAGE, 1, geometry=SourceGeometry(page_number=1), role="IMAGE")
    candidate = OcrCandidate("ocr:img", scope, 1, "standalone_image" if scope == "PAGE" else "embedded_image",
                             "img", SourceGeometry(page_number=1))
    return ParsedDocument("astra-indexator-document-v1", uuid4(), 1, src.sha256, src.detected_format,
                          ParserIdentity("fake", "v1", "default"), tuple(elements), (candidate,),
                          ParseQuality(QualityStatus.OCR_REQUIRED if scope == "PAGE" else QualityStatus.GOOD,
                                       len(native_text or ""), 1, (), (page_mode,)))
...
def test_if_needed_accepts_multilingual_ocr_and_preserves_provenance(tmp_path):
    ...
    result = OcrPipelineService(engine=engine, resolver=FakeResolver(src.local_path)).process(source=src, document=doc)
    assert [element.text for element in result.accepted_ocr_elements] == ["Құжат атауы", "Документ", "Document"]
...
def test_duplicate_ocr_is_not_appended_to_native_text(tmp_path):
    ...
    assert sum(1 for element in result.document.elements if element.text == "Клиенттің өтініші Document 42") == 1
...
def test_pixel_budget_rejects_before_engine(tmp_path):
    ...
    doc = parsed(src)
    engine = FakeEngine([("text", 0.9)])
    result = OcrPipelineService(engine=engine, resolver=FakeResolver(src.local_path, 1000, 1000)).process(
        source=src, document=doc, profile=OcrProfile(max_pixels_per_page=10)
    )
    assert engine.calls == 0
