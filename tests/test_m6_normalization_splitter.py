...
def _element(
    element_id: str, text: str | None, *, kind=ElementType.PARAGRAPH, order=0, **kwargs
) -> DocumentElement:
...
def test_table_cells_are_normalized_individually_and_empty_positions_survive():
    table = DocumentElement(
        ...
        ElementType.TABLE,
        0,
        metadata={"rows": [["  A ", ""], [" B   C ", None]], "rowCount": 2, "columnCount": 2},
    )
...
def test_one_off_edge_text_is_not_false_positive_furniture():
    elements = [
        _element(
            "x",
            "Important notice",
            geometry=SourceGeometry(page_number=1, y0=5, y1=20, page_height=1000),
        ),
        _element(
            "a",
            "A",
            order=1,
            geometry=SourceGeometry(page_number=2, y0=200, y1=220, page_height=1000),
        ),
        _element(
            "b",
            "B",
            order=2,
            geometry=SourceGeometry(page_number=3, y0=200, y1=220, page_height=1000),
        ),
    ]
    result = TextNormalizationService().normalize(_doc(elements))
...
def test_kazakh_abbreviations_do_not_create_false_boundaries():
    sentences = split_sentences(
        "Құжаттар т.б. материалдар тіркелді. Келесі бөлім.", language_hint="kk"
    )
...
def test_initials_versions_ip_and_url_are_protected_from_false_sentence_boundaries():
    ...
    assert sentences == (
        "A. Smith uses v1.2.3 at 10.20.30.40 and https://example.com/api.",
        "Done.",
    )
...
def test_oversized_paragraph_uses_sentence_or_forced_boundaries_and_never_exceeds_hard_chars():
    ...
    fragments = LogicalSplitter(
        _small_profile(hard_max_chars=100, soft_max_chars=80, target_chars=50)
    ).split(normalized)
    assert len(fragments) > 1
...
def test_table_continuation_repeats_header_as_synthetic_context_not_source_text():
    ...
    fragments = LogicalSplitter(
        _small_profile(hard_max_chars=90, soft_max_chars=70, target_chars=50)
    ).split(normalized)
    assert len(fragments) > 1
...
def test_list_intro_is_repeated_only_as_context_for_continuation():
    ...
    normalized = TextNormalizationService().normalize(_doc(elements))
    fragments = LogicalSplitter(
        _small_profile(target_chars=25, soft_max_chars=35, hard_max_chars=55)
    ).split(normalized)
