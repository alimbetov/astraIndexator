...
class Engine:
    ...
    def __init__(self, *, text="Текст", confidence=0.95, delay=0.0, error=None, geometry=None):
        ...
        self.delay = delay
        self.error = error
        self.geometry = geometry or SourceGeometry(
            page_number=1, x0=10, y0=20, x1=50, y1=40, coordinate_space="ocr-image-pixels"
        )
        self.calls = 0
...
class Resolver:
    ...
    def resolve(self, *, source, document, candidate, profile):
        ...
        return ResolvedOcrInput(
            ...
            self.source_geometry if self.source_geometry is not None else candidate.geometry,
            False,
        )
    ...
    *,
    ...
    elements.append(
        DocumentElement(
            ...
            ElementType.IMAGE,
            len(elements),
            geometry=geometry or SourceGeometry(page_number=1),
        )
    )
    candidate = OcrCandidate(
        candidate_id, scope, 1, reason, element_id, geometry or SourceGeometry(page_number=1)
    )
...
def with_candidates(doc: ParsedDocument, candidates, elements=None):
    return ParsedDocument(
        ...
        doc.detected_format,
        doc.parser,
        tuple(elements if elements is not None else doc.elements),
        tuple(candidates),
        ParseQuality(
            doc.quality.status,
            doc.quality.native_text_chars,
            len(candidates),
            doc.quality.warnings,
            doc.quality.page_modes,
        ),
    )
...
def test_unsupported_scope_is_rejected_without_rendering(tmp_path):
    ...
    resolver = Resolver(src.local_path)
    result = OcrPipelineService(engine=Engine(), resolver=resolver).process(
...
def test_invalid_pipeline_identity_fails_closed(tmp_path, mutation, error):
    ...
    if mutation == "hash":
        ...
        c = doc.ocr_candidates[0]
        missing = OcrCandidate(
            c.candidate_id, c.scope, c.page_number, c.reason, "missing", c.geometry, c.metadata
        )
...
def test_engine_pixel_geometry_is_normalized(tmp_path):
    ...
    result = OcrPipelineService(
        engine=Engine(), resolver=Resolver(src.local_path, width=100, height=100)
...
def test_region_geometry_maps_back_to_page_coordinates(tmp_path):
    ...
    engine = Engine(
        geometry=SourceGeometry(
            page_number=1, x0=0, y0=0, x1=50, y1=50, coordinate_space="ocr-image-pixels"
        )
    )
    result = OcrPipelineService(
        engine=engine,
        resolver=Resolver(src.local_path, width=100, height=100, source_geometry=region),
    ).process(source=src, document=doc)
    g = result.accepted_ocr_elements[0].geometry
...
def test_low_confidence_policy_keeps_evidence_above_hard_floor_and_drops_below(tmp_path):
    src = source(tmp_path)
    doc = document(src)
    retained = OcrPipelineService(
        engine=Engine(confidence=0.2), resolver=Resolver(src.local_path)
    ).process(
    ...
    dropped = OcrPipelineService(
        engine=Engine(confidence=0.05), resolver=Resolver(src.local_path)
    ).process(
        source=src, document=doc, profile=OcrProfile(min_confidence=0.35, hard_confidence_floor=0.1)
    )
...
def test_invalid_engine_confidence_is_classified_as_invalid_output(tmp_path):
    ...
    doc = document(src)
    result = OcrPipelineService(
        engine=Engine(confidence=1.5), resolver=Resolver(src.local_path)
    ).process(source=src, document=doc)
...
def test_render_failure_is_contained_and_classified(tmp_path):
    ...
    doc = document(src)
    result = OcrPipelineService(
        engine=Engine(), resolver=Resolver(src.local_path, fail=True)
    ).process(source=src, document=doc)
...
def test_engine_timeout_and_engine_failure_are_distinct(tmp_path):
    src = source(tmp_path)
    doc = document(src)
    timeout = OcrPipelineService(
        engine=Engine(error=TimeoutError("slow")), resolver=Resolver(src.local_path)
    ).process(source=src, document=doc)
    failure = OcrPipelineService(
        engine=Engine(error=RuntimeError("boom")), resolver=Resolver(src.local_path)
    ).process(source=src, document=doc)
...
def test_resource_guards_reject_before_engine(tmp_path, profile, expected):
    ...
    result = OcrPipelineService(
        engine=engine, resolver=Resolver(src.local_path, width=100, height=100)
    ).process(source=src, document=doc, profile=profile)
...
def test_soft_memory_limit_warns_but_allows_processing(tmp_path):
    ...
    result = OcrPipelineService(
        ...
        source=src,
        document=doc,
        profile=OcrProfile(memory_soft_limit_bytes=100, memory_hard_limit_bytes=100_000),
    )
...
def test_page_limit_does_not_count_rejected_page_as_processed(tmp_path):
    ...
    doc = ParsedDocument(
        ...
        base.source_sha256,
        base.detected_format,
        base.parser,
        (e1, e2),
        (c1, c2),
        ParseQuality(QualityStatus.OCR_REQUIRED, 0, 2, (), ("SCANNED_IMAGE", "SCANNED_IMAGE")),
    )
    engine = Engine()
    result = OcrPipelineService(engine=engine, resolver=Resolver(src.local_path)).process(
...
def test_total_pixel_budget_applies_across_candidates(tmp_path):
    ...
    base = document(src)
    elements = (
        DocumentElement("img1", ElementType.IMAGE, 0),
        DocumentElement("img2", ElementType.IMAGE, 1),
    )
    candidates = (
        OcrCandidate("ocr:1", "EMBEDDED_IMAGE", None, "image", "img1"),
    ...
    result = OcrPipelineService(
        engine=engine, resolver=Resolver(src.local_path, width=100, height=100)
    ).process(source=src, document=doc, profile=OcrProfile(max_total_pixels_per_job=15_000))
    assert engine.calls == 1
...
def test_png_region_is_cropped_before_ocr(tmp_path):
    src = source(tmp_path)
    region = SourceGeometry(
        page_number=1, x0=0.25, y0=0.25, x1=0.75, y1=0.75, coordinate_space="normalized-0-1"
    )
    doc = document(src, scope="REGION", geometry=region)
    resolver = DefaultOcrInputResolver(tmp_path / "work")
    resolved = resolver.resolve(
        source=src, document=doc, candidate=doc.ocr_candidates[0], profile=OcrProfile()
    )
    try:
...
def test_invalid_tiff_frame_is_explicit_render_failure(tmp_path):
    ...
    sha = hashlib.sha256(path.read_bytes()).hexdigest()
    src = AcquiredSource(
        StorageRef.parse("seaweed://docs/multi.tiff"),
        path,
        "multi.tiff",
        "TIFF",
        "image/tiff",
    ...
    doc = ParsedDocument(
        ...
        (element,),
        (candidate,),
        ParseQuality(QualityStatus.OCR_REQUIRED, 0, 1, (), ("SCANNED_IMAGE",)),
    )
    result = OcrPipelineService(
        engine=Engine(), resolver=DefaultOcrInputResolver(tmp_path / "work")
    ).process(source=src, document=doc)
    assert result.candidate_results[0].warnings == ("OCR_RENDER_FAILED",)
